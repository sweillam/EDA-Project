...
def get_user_input():
    num_nodes = simpledialog.askinteger("Input", "Enter the number of nodes:", parent=root, minvalue=2)
    nodes = []
    
    for i in range(num_nodes):
        x = simpledialog.askfloat("Input", f"Enter the x coordinate for Node {i+1}:", parent=root)
        y = simpledialog.askfloat("Input", f"Enter the y coordinate for Node {i+1}:", parent=root)
        nodes.append(Node(x, y))
    
    for i, node in enumerate(nodes):
        connected_nodes = simpledialog.askstring("Input", f"Enter the indices of the neighbours of {node.name} separated by commas (e.g., 1,2,3):", parent=root)
        if connected_nodes:
            for index in map(int, connected_nodes.split(',')):
                node.add_neighbour(nodes[index - 1])
